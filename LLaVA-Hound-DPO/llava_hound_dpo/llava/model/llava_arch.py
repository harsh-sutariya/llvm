...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        m = self.get_model().to(image_features.dtype)
        m = m.to(image_features.device)
        image_features = m.mm_projector(image_features) 
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        ## Xs[0] [2, 3, 336, 336]  keys ['image', 'image']
        ## Xs[0] [3, 336, 336]  keys ['image', 'image']
        ...
        # import pdb
        # pdb.set_trace()
        X_features = [getattr(self, f'encode_{key}s')(X) for X, key in zip(Xs, keys)]  # expand to get batchsize
        # X_features = [getattr(self, f'encode_{key}s')(X.unsqueeze(0)) for X, key in zip(Xs, keys)]  # expand to get batchsize
        ### X_features[0] torch.Size([1, 576, 4096])
        ### X_features[0] torch.Size([2, 576, 4096])
        ...
        #### lzy：这个地方注释掉flatten的代码，多图不需要flatten
        # X_features = [x.flatten(0, 1) for x in X_features]
        ### X_features[0] torch.Size([576, 4096])
        ### X_features[0] torch.Size([1152, 4096]) torch.Size([2, 576, 4096])
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # print(4444444444)
            #### 多图处理（lzy modified）
            count_all_images = X_token_indices.numel()
            count_image = 0
            # pdb.set_trace()
            while X_token_indices.numel() > 0:
                if count_all_images>1:
                    cur_X_features = X_features[batch_idx][count_image]
                    count_image+=1
                #### 此处添加一个 else， 对于单图的数据，也走多图的通道，单数数据需要使用list格式
                else:
                    cur_X_features = X_features[batch_idx][count_image]
                    count_image+=1
                # cur_X_features = X_features[cur_X_idx]
            ...
            #### cur_new_input_embeds[0].shape
            #### cur_new_input_embeds[1].shape
            #### cur_new_input_embeds[2].shape
            #### cur_new_input_embeds[3].shape
            #### cur_new_input_embeds[4].shape
            #### cur_new_input_embeds[5].shape
