...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, legacy=False, bf16 = True):
    ...
    print("model_name: ", model_name.lower())
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            mm_projector_weights = {k: v.to(torch.bfloat16 if bf16 else torch.float16) for k, v in mm_projector_weights.items()}
        ...
        if model_base is not None:
            ...
            from peft import PeftModel
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, torch_dtype=torch.bfloat16 if bf16 else torch.float16, low_cpu_mem_usage=True, device_map="auto")
            print(f"Loading LoRA weights from {model_path}")
            model = PeftModel.from_pretrained(model, model_path)
            print(f"Merging weights")
            model = model.merge_and_unload()
            print('Convert to FP16...')
            model.to(torch.bfloat16 if bf16 else torch.float16)
        else:
            use_fast = False
    ...
    if 'llava' in model_name.lower():
        ...
        if 'Image' in X:
            ...
            image_tower.to(device=device, dtype= torch.bfloat16 if bf16 else torch.float16)
        ...
        if 'Video' in X:
            ...
            video_tower.to(device=device, dtype=torch.bfloat16 if bf16 else torch.float16)
