...
def eval_model(args):
    ...
    for example in tqdm( test_data ):
        ...
        for image in images:
            image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0].half()
            image = image.reshape(-1, 3, image.shape[-2], image.shape[-1])
            processed_images.append(image)

        image_tensor = torch.cat(processed_images).to(model.device) # probably mention the model.device
