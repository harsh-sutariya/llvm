...
class DPOTrainer(Trainer):
    ...
    def ratio_sequence(self, input_ids, attention, num_of_images, target_number):
        layer_number = 16 
        ...
        max_tesor_size = len(output_token_attention) + len(attention[0][layer_number][0])
        merged_tensor = torch.zeros(len(attention[0][layer_number][0]), max_tesor_size, max_tesor_size)

        merged_tensor[:, :len(attention[0][layer_number][0]), :len(attention[0][layer_number][0])] = input_attention


        for i in range(1, len(output_token_attention)+1):
            merged_tensor[:, len(attention[0][layer_number][0]) + i - 1, :len(attention[0][layer_number][0]) + i] = output_token_attention[i-1].squeeze(1)
    ...
    def ratio_pip(self, input_ids, attention):
        layer_number = 16
        ### 找到所有 <image> 的起始位置 
        indices = torch.nonzero(input_ids[0] == -200, as_tuple=False).squeeze().tolist()
    ...
    def ratio_collage(self, input_ids, attention, image_dims, block_index):
    ...
    def calculation_attention_ratio(self, input_ids, attention, num_of_images, target_number, image_type, image_dims):
        

        input_ids = input_ids.unsqueeze(0)

        if image_type == "sequence":
            return self.ratio_sequence(input_ids, attention, num_of_images, target_number)

        elif image_type == "collage":
            return self.ratio_collage(input_ids, attention, image_dims, target_number)
        elif image_type == "pip":
            return self.ratio_pip(input_ids, attention)
        else:
    ...
    def concatenated_forward(
        ...
        all_logits, new_labels, attentions = model(
        ...
        chosen_attentions = [attention[:len_chosen] for attention in attentions] if attentions else None
        rejected_attentions = [attention[len_chosen:] for attention in attentions] if attentions else None
        ...
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_labels, rejected_labels, chosen_attentions, rejected_attentions)

    def attention_loss(self, attention_ratios, threshold=0.75):
        """
        Compute the attention loss as max(0, threshold - attention_ratio)
        
        Args:
            attention_ratios: List of attention ratios (one per batch item)
            threshold: Minimum acceptable attention ratio
            
        Returns:
            torch.Tensor containing the attention loss
        """
        losses = []
        ...
        return torch.tensor(losses, device=self.accelerator.device, dtype=torch.float32).mean()
    ...
    def get_batch_loss_metrics(
        ...
        target_numbers = batch['target_number']
        image_types = batch["image_type"]
        image_dims = batch["image_dims"]
        attention_loss = 0.0
        attention_ratios = []

        for batch_idx in range(batch['chosen_input_ids'].shape[0]):
            attention_ratios.append(self.calculation_attention_ratio(
                batch["chosen_input_ids"][batch_idx],
                chosen_attentions[batch_idx],
                num_of_images[batch_idx],
                target_numbers[batch_idx],
                image_types[batch_idx],
                image_dims[batch_idx]
            ))

        unscaled_attention_loss = self.attention_loss(attention_ratios, threshold=self.attention_threshold)
        ...
        losses = dpo_losses + sft_loss + attention_loss
        ...
        unscaled_attention_loss = all_gather_tensor(unscaled_attention_loss)
        ...
        # Add attention metrics for logging
        metrics[f"{prefix}losses/attention"] = unscaled_attention_loss.mean().cpu()
