...
def calculation_attention(input_ids, attention, is_draw):
    ### 设定输出的attention层数
    layer_number = 16
    ### 找到所有 <image> 的起始位置 
...
def calculate_center_ratio(array):
    # 确保输入数组是24x24的
    if array.shape != (24, 24):
    ...
    # 提取中间16x16区域
    center_region = array[start:end, start:end]
...
selected_pip_data = pics_in_pics_49k
local_index = 5
selected_pip_data = selected_pip_data[40000:]
print(len(selected_pip_data))

count = 0
dpo_data = []
for item in tqdm(selected_pip_data):
    try:
        ### 预处理数据
        images = item['image']
        ...
        blur_images = process_images_with_noise([images], noise_std=49)
        ...
        if len(gt)<512:
            ...
            ratio = calculate_center_ratio(final_tensor_np)
            # print(ratio)
            ...
            ### 显示低于阈值的数据
            if gt!=response:
                if ratio<0.8 and ppl>1.1:
                    # plot_images([images])
                    # fig, axes = plt.subplots(1, 1, figsize=(15, 5))
                    # axes.imshow(blur_images[0])
                    # axes.axis('off')
                    # plt.show()
                    count+=1
                    print(count)
...
file_path = f"./pics_in_pics_{local_index}.json"
print(len(dpo_data))
